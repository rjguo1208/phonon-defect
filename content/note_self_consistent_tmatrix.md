...
For a dilute random array of identical defects (density $n_d$) the configuration-averaged Green's function is $\bar G(\omega)=[\,\omega-H_0-\Sigma(\omega)\,]^{-1}$; the physics is in how the disorder self-energy $\Sigma$ is built from the single-defect potential $v$. This note derives the standard hierarchy and writes the **self-consistent $T$-matrix (SCTMA)** equations in the active/rest-downfolded representation we use, so it plugs directly into $T=[1-\tilde V G^A]^{-1}\tilde V$. It also states why SCTMA is the dilute limit of CPA. *(The V$_S$ $a_1$ mis-placement once used as a test case for SCTMA was subsequently traced to a deep-band rest-truncation, fixable inside the $T$-matrix — see the §9 update — so SCTMA's real role is the finite-$n_d$ broadening of the deep $e$, not the $a_1$.)*
...
- **Beyond reach.** SCTMA is still **non-crossing**: it omits inter-defect interference (Anderson localization, coherent backscattering) and $O(n_d^2)$ cluster effects, and it is *not* DFT self-consistency (no charge-density / $\Delta V$ update). *(Note: the V$_S$ $a_1$ mis-placement used as the test case below turned out to be a deep-band rest-truncation, fixable inside the $T$-matrix — see the §9 update — not a self-consistency limitation.)*
...
**Verdict.** SCTMA does **not** move the $a_1$: at the physical density it shifts by only $-9$ meV (negligible), staying pinned at the band edge — exactly the $\mathcal O(n_d)$ smallness the §8 estimate predicted. What SCTMA *does* produce at $n_d=2.78\%$ is a **disorder broadening of the deep $e$ level** (a finite-concentration lifetime), not a change to the band-edge state.

**Update — the real cause of the mis-placed $a_1$ (this changes the interpretation).** This test used the original rest space $Q=18$–70, which **omits the deep bands 1–6**. That omission — *not* any self-consistency deficiency — is what put the $a_1$ on the valence edge ([$T$-matrix page §4.1](tmatrix.html)): a full diagonalization (bands 1–66) gives $a_1=+0.238$, but excluding 1–6 drops it below the VBM. Restoring them, $Q=\{1\text{–}6\}\cup\{18\text{–}66\}$, recovers $a_1=+0.238$ (= full diagonalization = DFT $\approx+0.24$) **within the non-self-consistent $T$-matrix**. So the correct reading of this section is: SCTMA leaves the $a_1$ unchanged because the $a_1$ problem is a **rest-space completeness** problem, not a self-consistency one — fixed by adding the deep bands, with neither SCTMA nor a DFT supercell required. *(The earlier conclusion that the $a_1$ "needs DFT-supercell self-consistency" is retracted.)* SCTMA's genuine, separate role is the finite-$n_d$ disorder broadening of the deep $e$.
