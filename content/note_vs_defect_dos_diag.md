...
![O_S vs Se_S perturbation potential](../assets/dV_compare_os_se.png)
*A second comparison — two **isovalent** chalcogen substitutions (group VI: O, S, Se) in the same frozen 6×6 cell. **The perturbation flips sign and scale between them:** O$_S$ (O smaller and more electronegative than S, same +6 pseudo-ion) gives a *positive, modest* $\Delta V$ (+34 eV peak, red); Se$_S$ (Se larger than S) gives a *strongly negative* $\Delta V$ (−167 eV peak, blue — ~5× larger). The deep Se$_S$ core well is partly chemical (Se's larger, deeper ion) and partly pseudopotential: the dojo-stringent Se carries a 3s3p3d semicore (pseudo-ion +24 vs S's +6). So even "isovalent" substitutions span a huge range of $\Delta V$ sign and magnitude. Combined with the V$_S$/O$_S$ panel above, this underscores that the in-gap outcome is set by defect chemistry — whether a real bound state forms — not by the size of $\Delta V$. (Frozen geometry; a real Se$_S$ would relax outward. Semicore Se pseudo chosen for consistency with the host set.)*
